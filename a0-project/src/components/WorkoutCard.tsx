...
const VolumeVisualizer = ({
  ...
  const volumeInterpolations = useRef(
    volumeAnims.map((anim) =>
      anim.interpolate({
        inputRange: [0, 1],
        outputRange: ["40%", "90%"],
      }),
  ...
  useEffect(() => {
    const animations = volumeAnims.map((anim, index) => {
      // Reset value before starting loop to ensure consistent start state
      anim.setValue(0);
  ...
  return (
    <View style={styles.volumeSegmentsContainer}>
      {volumeAnims.map((anim, index) => {
        ...
        const isActive = localVolume >= targetVolume - 0.05;

        const animatedHeight = volumeInterpolations[index];

        return (
          <Pressable
            key={level}
            style={styles.volumeSegmentBox}
            onPress={() => onVolumeChange(targetVolume)}
          >
            <Animated.View
              style={[
...
const MusicControl = React.memo(
  ({
    ...
    onToggleMusic: () => void;
    selectedTrack: MusicTrackKey;
    onNextTrack: () => void;
    onPrevTrack: () => void;
    onVolumeChange: (vol: number) => void;
    onShowInfo: () => void;
  }) => {
    // Local volume state for UI responsiveness
        ...
        <View style={styles.musicHeader}>
          <View style={styles.musicHeaderLeft}>
            <MaterialIcons
              name="music-note"
              size={24}
              color={isMusicEnabled ? "#4CAF50" : "#c5c5c5"}
            />
            <Text style={[styles.musicLabel, { color: isMusicEnabled ? "#FFFFFF" : "#c5c5c5" }]}>
              {isMusicEnabled ? "배경 음악 켜짐" : "배경 음악 꺼짐"}
            </Text>
          </View>
          <Pressable onPress={onToggleMusic} hitSlop={10}>
        ...
        {isMusicEnabled && (
          <View style={styles.musicBody}>
            ...
            <View style={styles.trackControl}>
              <Pressable onPress={onPrevTrack} style={styles.trackButton}>
                <MaterialIcons name="skip-previous" size={32} color="#FFFFFF" />
              </Pressable>

              <Pressable style={styles.trackInfoContainer} onPress={onShowInfo}>
              ...
              <Pressable onPress={onNextTrack} style={styles.trackButton}>
            ...
            <View style={styles.volumeControl}>
              <Pressable onPress={() => handleVolumeChange(0)} hitSlop={10}>
                <MaterialIcons
                  name={localVolume === 0 ? "volume-off" : "volume-mute"}
                  size={24}
                  color="#AAAAAA"
                />
              </Pressable>

              <VolumeVisualizer localVolume={localVolume} onVolumeChange={handleVolumeChange} />

              <Pressable onPress={() => handleVolumeChange(1)} hitSlop={10}>
...
export default function WorkoutCard({
  ...
  // Store volume in Ref to avoid re-renders of WorkoutCard on volume change
  ...
  const handleVolumeChangeFromControl = async (newVol: number) => {
    volumeRef.current = newVol;
    if (backgroundMusic) {
      try {
        await backgroundMusic.setVolumeAsync(newVol);
      } catch (e) {
    ...
    <>
      ...
      >
        <Pressable onPress={handlePress} style={styles.pressableArea}>
          <View style={styles.header}>
            ...
            <View style={styles.actions}>
              {isTimerActive && !isPaused ? (
                <MaterialIcons name="pause" size={24} color="#ffffff" style={styles.statusIcon} />
              ) : isTimerActive && isPaused ? (
                <MaterialIcons
                  name="play-arrow"
                  size={24}
            ...
            <Progress.Bar
              ...
              progress={
                getDurationForMode(timerMode) > 0
                  ? (getDurationForMode(timerMode) - displayTime) / getDurationForMode(timerMode)
                  : 0
              }
        ...
        <MusicControl
          ...
          onVolumeChange={handleVolumeChangeFromControl}
          onShowInfo={() => setIsMusicInfoModalVisible(true)}
        />
      </View>
            ...
            <ScrollView style={styles.musicInfoContent}>
              {Object.entries(musicInfo).map(([track, info]) => (
                <View key={track} style={styles.musicInfoItem}>
                  ...
                  <Text style={styles.musicInfoText}>✔ Music provided by {info.provider}</Text>
